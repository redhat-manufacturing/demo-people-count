...
class VideoReader:
    ...
    def __init__(self,
        ...
        if not str(source).startswith('rtsp') and not str(source).startswith('http') or str(source).isnumeric():
            # print ("LLL")
    ...
    def start_stream(self):
        ...
        # set new FPS, Width, Height if given
        if str(self._stream_args["source"]).isnumeric():
        ...
        if str(self._stream_args["source"]).isnumeric():
            self._fps = int(self._stream.get(cv2.CAP_PROP_FPS))
        # self._fps =  int(self._stream_args['fps'])
